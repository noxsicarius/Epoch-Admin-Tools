...
if(isNull(_target)) then {
	...
	cutText [format["%1 humanity has been added (total: %2) for player %3", humanityGain, _humanity + humanityGain, name _player],"PLAIN DOWN"]; titleFadeOut 10;
		
	// Tool use logger
	if(logMinorTool) then {
		usageLogger = format["%1 %2 -- has added %3 to their humanity (total %4)",name _player,getPlayerUID _player,humanityGain,_humanity + humanityGain];
