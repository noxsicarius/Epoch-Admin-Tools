...
F9_KEY = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) == 67) then {toolsAreActive=true;};"];
Sleep 0.02;

// Used to toggle admin scroll menu OFF
F10_KEY = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) == 68) then {toolsAreActive=false;};"];
Sleep 0.02;


// Used to launch the admin GUI menu
F11_KEY = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) == 87) then {[] execVM ""admintools\AdminMainGUI.sqf"";true};"];
Sleep 0.02;
